...
LastEditTime: 2023-01-12 17:35:43
'''
#coding=utf-8
import sys
import threading
...
from PyQt5.QtWidgets import (QGridLayout, QPushButton, QFrame, QSplitter,
                             QHBoxLayout, QScrollArea, QLabel, QComboBox,
...
class Window_1(QMainWindow, Ui_MainWindow):
    ...
    def load_browser(self, w, listf):
        ...
        m = folium.Map(location=[35.3, 100.6],
                       zoom_start=4,
                       attr='高德-常规图',
                       tiles=tiles,
                       max_bounds=True,
        ...
        for i, l in enumerate(listf):
            mak = folium.Marker(list(map(lambda x: float(x), [l[3], l[2]])),
                                tooltip=l[1],
    ...
    def retrieve_task(self):
        ...
        sigs = [
            sig.win1_factory_info_retrive_done, sig.win1_online_retrive_done,
            sig.win1_alert_retrive_done
        ]
        widgets = [
            self.browser, self.list_widget_online, self.list_widget_alert
        ]
        ret = utils.Retriever()
